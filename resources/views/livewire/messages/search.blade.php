...
<x-filament::modal width="xl" id="search-conversation">
    <x-slot name="heading">
        {{__('filament-messenger::messages.search_messages')}}
    </x-slot>

    <x-filament::input.wrapper suffix-icon="heroicon-o-magnifying-glass">
        <x-filament::input type="search" placeholder="{{__('filament-messenger::messages.search_placeholder')}}" wire:model.live.debounce.500ms="search"/>
            ...
            <ul class="bg-white dark:bg-gray-900 divide-y dark:divide-white/10">
                @foreach($messages as $message)
                    <li wire:key="{{ $message->id }}" class="hover:bg-gray-100 dark:hover:bg-white/10">
                        <a wire:navigate href="{{ Messages::getUrl(tenant: filament()->getTenant()) . '/' . $message->inbox->id . '?tab=' . $message->inbox->getTabForCurrentUser() }}">
                            <div class="grid grid-cols-[--cols-default] lg:grid-cols-[--cols-lg] p-3" style="--cols-default: repeat(1, minmax(0, 1fr)); --cols-lg: repeat(6, minmax(0, 1fr));">
                                <div style="--col-span-default: span 4 / span 4;" class="col-[--col-span-default]">
                                    <div class="flex gap-3">
                                        @php
                                            $otherUserName = $message->inbox->other_users->first()?->name ?? $message->inbox->inbox_title;
                                        ...
                                        <div class="overflow-hidden">
                                            ...
                                            <p class="text-sm text-gray-600 truncate dark:text-gray-400">
                                                @if ($message->user_id == auth()->id())
                                                    <span class="font-bold">{{ __('filament-messenger::messages.you') }}:</span>
                                                @else
                                ...
                                <div style="--col-span-default: span 2 / span 2;" class="col-[--col-span-default]">
                                    <p class="text-sm font-light text-gray-600 dark:text-gray-500 text-end">
                                        {{ \Carbon\Carbon::parse($message->updated_at)->setTimezone(config('filament-messenger.timezone', 'app.timezone'))->locale('fr')->isoFormat('D MMMM YYYY') }}
                                    </p>
                                </div>
                            </div>
                        </a>
        ...
        <div class="relative">
           <div class="absolute z-10 w-full bg-white border dark:divide-gray-800 border-gray-200 rounded-lg shadow dark:border-gray-800 dark:bg-gray-900 max-h-64 overflow-y-auto">
                <p class="w-full p-3 text-sm text-center text-gray-500 dark:text-gray-400">
                    {{__('filament-messenger::messages.no_results_found')}}
                </p>
            </div>
        </div>
    @endif
